...
class manager(object):
  def __init__(self, individual_sites       = False,
    ...
    self.adp_nm                 = self._deep_copy(self.adp_nm)
  ...
  def deep_copy(self):
    return manager(
      ...
      adp_nm                 = self._deep_copy(self.adp_nm),
  ...
  def check_all(self):
    ...
    if(self.nm):
      if(not self._count_selected(self.adp_nm)):
        raise Sorry(prefix%"adp_nm.")
  ...
  def show(self, log = None):
    ...
    print >> log, "  nm                     = %5s (%s atoms in %s groups)" % (
      str(self.nm), self.ca(self.adp_nm), self.szs(self.adp_nm))
  ...
  def select(self, selection):
    ...
    if(self.adp_nm is not None):
      self.adp_nm = self._select(self.adp_nm, selection)
  ...
  def inflate(self, sites_individual       = None,
    ...
    if(adp_nm is not None):
      assert hasattr(adp_nm, 'count')
